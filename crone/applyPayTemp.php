        ...
        if($app->ValidateAuthCode($DataPayment[$i+4])=='') // si no existe el codigo de autorizacion
        {
            ...
            {   //abono a tarjeta
                ...
                else
                {
                    ...
                    '<tr><td> Motivo Error </td><td> Falla registro Movimiento API HOTPAY </td></tr></table>';
                    $email->enviarmail($mail_mensaje,'ERROR DE REGISTRO HOTPAY FONDEO TARJETA');
                }
            }
            elseif($DataPayment[$i+6]=2) //cargo a tarjeta(Reverso a tarjeta)
            {
                ...
                else
                {
                    ...
                    '<tr><td> IdMovement </td><td> '.$DataPayment[$i].'</td></tr>'.
                    '<tr><td> Motivo Error </td><td> Falla registro Movimiento API HOTPAY </td></tr></table>';
                    $email->enviarmail($mail_mensaje,'ERROR DE REGISTRO HOTPAY REVERSO TARJETA');
                }
            }
        }
        else
