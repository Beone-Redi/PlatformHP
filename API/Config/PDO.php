...
class PDOSS
{
    ...
    public function __construct()
    {
        try 
        {
            ...
            $this->clientKey    = 'GG2SZ7AJ34WSHOK1KI55Y8';  //key  
    ...
    public function RegisterCard($DataCard)
    {
        try
        {
            ...
            if($stmt->execute())
            {
                ...
                /* Modulo Unico GNT 
                //Envio de correo si da de alta una tarjeta.
                if($DataCard[8]==3)
                {
                    $email        = new sendmail();
                    $mail_mensaje='<table>
                        <tr><td> Tarjeta </td><td> 51291201'.$DataCard[0].'</td></tr>'.
                        '<tr><td> Motivo mensaje </td><td> '.'SOLICITAR ALTA TARJETA EN ONE CARD PARA COMPRAS Y USO EN CAJERO'.'</td></tr></table>';
                    $email->enviarmail($mail_mensaje,'ALTA TARJETA API WIREBIT');
                }*/   
            }
            else
            {
    ...
    public function SaveDatapayment( $Data )
    {
        ...
        if(sizeof($movement_duplicated)>0)
        {
            ...
            '<tr><td> Motivo Error </td><td> '.'POSIBLE INTENTO DE COBRO DUPLICADO HOTPAY'.'</td></tr></table>';
            $email->enviarmail($mail_mensaje,'INTENTO DE COBRO DOBLE PAYNET HOTPAY');
