...
if ( $app->optSearch($_SESSION[ 'PERMISOS' ],'opt26') ) // si tiene el permiso puede verlo si no no
{
    ...
    if ( isset($_POST["inputNombreCompleto"]))
    {
        ...
        $IdProductCard=$_POST['inputtypecard'];
        if($inputClave1===$inputClave2)
        {
            ...
            if($bandera==0) // si no ocurrio un error al cargar la imagen de perfil
            {
                ...
                if(substr( $Resultado1, 0, 2 ) <> '01') // si se creo correctamente el usuario
                {
                    ...
                    $Data=[$IdUserAPI,$IdCard,$Month,$Anio,$Company,$Usuario,$City,$IdProductCard];//Idcard,mes,anio,compañia,usuario,ciudad
    ...
    {
        $cards=[];
        $string_card=$app->GetCardsCompanyByIde($_SESSION['EMPRESA']);
        if($string_card!='')
        {       
            $cards=explode('-',$string_card);
        }
        ...
        $csv .= 'Tarjeta,Nombre,MV,AV,correo,Contraseña,Usuario,IdTarjeta,Respuesta 1,Respuesta 2'. $csv_end; // formato de respuesta
                            ...
                            while ( ( $datos = fgetcsv( $fichero, 1000 ) ) !== FALSE ) 
                            {
                                if ($NumFilas>0) 
                                {
                                    if(sizeof($datos)==8)
                                    {
                                        ...
                                        elseif(sizeof($cards)<1)
                                        {
                                            $Respuesta1=$Respuesta2='01TIPO DE TARJETA NO VALIDO';
                                        }
                                        elseif(!(in_array($datos[7], $cards)))
                                        {
                                            $Respuesta1=$Respuesta2='01TIPO DE TARJETA NO VALIDO 2';
                                        ...
                                        else
                                        {
                                            ...
                                            if($band_correo==0)
                                            {
                                                ...
                                                if(substr( $Respuesta1, 0, 2 ) <> '01') // si se creo correctamente el usuario
                                                {
                                                    ...
                                                    $City=utf8_decode($inputCiudad);
                                                    $dataComision=[$ComisionAdmin[0],$ComisionAdmin[1],0.00,0.00];//Comisienadmin,/IvAcomisionAdmin,
                                                    $Data=[$IdUserAPI,$IdCard,$Month,$Anio,$Company,$Usuario,$City,$datos[7]];//Idcard,mes,anio,compañia,usuario,ciudad
                                        ...
                                        {
                                            $csv.=$datos[0].$csv_sep.$datos[1].$csv_sep.$inputMes.$csv_sep.$inputAno.$csv_sep.$correo_csv.$csv_sep.$password_csv.$csv_sep.$user.$csv_sep.$datos[7].$csv_sep.substr($Respuesta1,2).$csv_sep.substr($Respuesta2,2).$csv_end;
                                            $num_registros++;
                                        }
                                        else
                                        {
                                            $csv.=$datos[0].$csv_sep.$datos[1].$csv_sep.$datos[2].$csv_sep.$datos[3].$csv_sep.$datos[4].$csv_sep.$datos[5].$csv_sep.$datos[6].$csv_sep.$datos[7].$csv_sep.substr($Respuesta1,2).$csv_sep.substr($Respuesta2,2).$csv_end;                                    
                                            $error_registros++;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        $csv.='0,0,0,0,0,0,0,0,0,ERROR EN EL NUMERO DE CAMPOS'.$csv_end;
                                    }  
                                }
                                $NumFilas++;    
                            }   
                ...
                <div class="card">
                    ...
                    <div class="content">
                        <form action="create_user?scr=9" method="POST" enctype="multipart/form-data" onsubmit="return checkSubmitBlock('btsubmit');" />
                            ...
                            <div class="row">        
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" min="1000" max="99998" class="form-control" name="inputCodigoPostal" value="<?php echo $value_cp; ?>" maxlength="5" required oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" />
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-5">
                                    <label for="inputCard">Tarjeta</label>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="inputCodigoPostal">Tipo de tarjeta </label>
                                        <select class="form-control" name="inputtypecard" required>
                                            <option value=""></option>
                                        <?php for($i=0;$i<sizeof($cards);$i++)
                                        {
                                            $label=$app->getlabelProductById($cards[$i]);
                                            ?>
                                            
                                            <option value="<?php echo $cards[$i];?>"><?php echo $cards[$i].'  -  '.$label;?></option>
                                        <?php
                                        }?>
                            ...
                            <button type="submit" id="btsubmit" class="btn btn-info btn-fill pull-right">Guardar</button>
                            &nbsp;
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="content">     
                        <form action="create_user?scr=9" method="POST" id="form2" enctype="multipart/form-data" onsubmit="return checkSubmitBlock('btsubmit3');">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
