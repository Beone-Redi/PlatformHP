...
class msjs_mails
{
    ...
    public function Found_card_Manual($Data)
    {
        //Data=[empresa,saldo cuenta maestra,saldo empresa,Monto fondeo,Tarjeta,saldo_tarjeta,Accion]
        $msj_aplicacion_pagos=
            '<br> <b><h3>Empresa '.strtoupper($Data[0]).'</b></h3>-----------------------<br>';
            $table_mensaje='<table>
                 <tr><td> Accion </td><td> '.$Data[3].'</td></tr>'.
                '<tr><td> Tarjeta </td><td> ****-****-'.str_pad(substr($Data[2], 0,-4),4, "**", STR_PAD_LEFT).'-'.substr($Data[2], -4).'</td></tr>'.
                '<tr><td> Monto del fondeo </td><td>$ '.number_format($Data[1],2,'.',',').'</td></tr>'.
                '<tr><td> Descripción </td><td> '.$Data[4].'</td></tr>'.
                '<tr><td> Motivo </td><td> '.$Data[5].'</td></tr></table>';
        $msj_aplicacion_pagos.=$table_mensaje; // Creacion del mensaje a enviar por correo
        return  $msj_aplicacion_pagos; 
    }
