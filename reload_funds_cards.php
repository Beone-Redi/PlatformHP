                    ...
                    if ($NumFilas>0) 
                    {
                        if(sizeof($datos)==2) // Si solo trae el credito
                        {
                            ...
                            else
                            {
                                ...
                                $saldo_tarjeta=$app->check_amountCard($tarjeta); // Verifica el saldo de la tarjeta
                                    ...
                                    { 
                                        ...
                                        else
                                        {
                                            if( $validar_tarjeta>2 )
                                            {
                                                ...
                                                $descripcion='TRES INTENTOS DE FONDEO A TARJETA HOTPAY NO COMPLETADOS';   
                                                $titulo_correo='NOTIFICACION DE FONDEOS A TARJETA HOTPAY NO COMPLETADOS'; 
                                            ...
                                            else
                                            {
                                                ...
                                                if(substr( $Respuesta, 0, 2 ) !== '00') // Error de respuesta de la API
                                                {
                                                    ...
                                                    if( $saldo_tarjeta_new == ($saldo_tarjeta+$monto_fondeo) ) // Si el saldo actual es mayor al saldo anterior en la tarjeta
                                                    {
                                                        ...
                                                        $titulo_correo='NOTIFICACION DE PAGOS LAYOUT A TARJETA HOTPAY';
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        $titulo_correo='NOTIFICACION DE FONDEO POR LAYOUT A TARJETA HOTPAY NO EFECTUADO';
                                                ...
                                                {
                                                    $titulo_correo='NOTIFICACION DE FONDEO LAYOUT A TARJETA HOTPAY';
                                                    $respuesta_mail=$email->enviarmail($msj_aplicacion_pagos,$titulo_correo);
                                                    $datalogmail=[1,$msj_aplicacion_pagos,$respuesta_mail];
                                                    $app->insertlogemail($datalogmail);  
                                                }
...
elseif ( isset( $_POST['inputPerfil'] ) AND $_POST['inputPerfil']>0 AND isset($_SESSION["EMPRESA"]) AND !is_null($_SESSION["EMPRESA"]))
{
    ...
    $tarjeta        = $_POST["inputPerfil"]; // tarjeta a 8 digitos
                    ...
                    else
                    {
                        ...
                        if(substr( $texto, 0, 2 ) !== '00') // si ocurre algun error en la aplicacion del pago
                        {
                            ...
                            if($saldo_tarjeta_new==($saldo_tarjeta+$monto_fondeo)) // Si el saldo actual es mayor al saldo anterior en la tarjeta
                            {
                                ...
                                $app->UpdateCardIncidents($idincident,$tarjeta); // actualiza el estatus para liberar la tarjeta
                                $titulo_correo='NOTIFICACION DE PAGOS A TARJETA HOTPAY';
                                $respuesta_mail=$email->enviarmail($msj_aplicacion_pagos,$titulo_correo);
                                $datalogmail=[1,$msj_aplicacion_pagos,$respuesta_mail];
                                $app->insertlogemail($datalogmail);  
                            }
                            else
                            {
                                ...
                                $titulo_correo='NOTIFICACION DE FONDEO A TARJETA HOTPAY NO EFECTUADO';
                        ...
                        {
                            $titulo_correo='NOTIFICACION DE FONDEO A TARJETA HOTPAY';
                            $respuesta_mail=$email->enviarmail($msj_aplicacion_pagos,$titulo_correo);
                            $datalogmail=[1,$msj_aplicacion_pagos,$respuesta_mail];
                            $app->insertlogemail($datalogmail);
                        }
        ...
        if($monto_fondeo<=$saldo_tarjeta)
        {
            ...
            else
            {
                if( $validar_tarjeta>=3)
                {
                    ...
                    $descripcion='TRES INTENTOS DE REVERSO A TARJETA HOTPAY NO COMPLETADOS';   
                    $titulo_correo='NOTIFICACION DE REVERSOS A TARJETA HOTPAY NO COMPLETADOS'; 
                    $dataerror=[$datacompany[1],$accion,$tarjeta,$monto_fondeo,$descripcion];                                
                    $msj=$msjs_mails->Error_Found_card($dataerror); // construye el mensaje a enviar  
                    $respuesta_mail=$email->enviarmail($msj,$titulo_correo);
                    $datalogmail=[1,$msj,$respuesta_mail];
                ...
                else
                {
                    ...
                    if(substr( $texto, 0, 2 ) !== '00')
                    {
                        ...
                        if($saldo_tarjeta_new==($saldo_tarjeta+$monto_fondeo)) // Si el saldo actual es mayor al saldo anterior en la tarjeta
                        {
                            ...
                            $app->UpdateCardIncidents($idincident,$tarjeta); // actualiza el estatus para liberar la tarjeta
                            $titulo_correo='NOTIFICACION DE REVERSO A TARJETA HOTPAY';
                            $respuesta_mail=$email->enviarmail($msj_aplicacion_pagos,$titulo_correo);
                            $datalogmail=[1,$msj_aplicacion_pagos,$respuesta_mail];
                            $app->insertlogemail($datalogmail);  
                        }
                        else
                        {
                            ...
                            $titulo_correo='NOTIFICACION DE REVERSO A TARJETA HOTPAY NO EFECTUADO';
                            //Data=[empresa,Monto,Tarjeta,Accion,concepto]
                            $dataerror=[$datacompany[1],$accion,$tarjeta,$monto_fondeo,$descripcion];                                            
                            $msj=$msjs_mails->Error_Found_card($dataerror); // construye el mensaje a enviar  
                            $respuesta_mail=$email->enviarmail($msj,$titulo_correo);
                ...
                <div class="card">
                    ...
                    <div class="content">
                        <form action="reload_funds_cards?scr=4" method="POST" enctype="multipart/form-data" id="form1" onsubmit="return checkSubmitBlock('btsubmit3');">
                            ...
                            <button type="submit" id="btsubmit3" class="btn btn-info btn-fill pull-right" form="form1">Fondear</button>
                    ...
                    <div class="content">
                        <form action="reload_funds_cards?scr=4" method="POST" id="form2" onsubmit="checkSubmitBlock('btsubmit');">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="inputObservaciones">Comentario</label>
                                        <textarea rows="5" class="form-control" name="comentario" maxlength="100"></textarea>
                                    </div>
                                </div>
                            </div>
                            
                            <button type="submit" id="btsubmit" onclick="confirm_form('amount');" class="btn btn-info btn-fill pull-right" form="form2">Fondear</button>
                            <div class="clearfix"></div>
                        </form>
                    </div>
                </div>
