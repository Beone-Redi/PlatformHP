        ...
        <div class="row">
            ...
            <?php if ( strlen( $msg ) == 6 ) { ?> 
                <div class="alert alert-success">
                    ...
                    <span><b> Exitoso - </b>Empresa : <?php echo strtoupper($empresa);?> Rangos solicitados: <?php echo substr($fecha_ini,8,2).'/'.substr($fecha_ini,5,2).'/'.substr($fecha_ini,0,4) . ' y ' . substr($fecha_fin,8,2).'/'.substr($fecha_fin,5,2).'/'.substr($fecha_fin,0,4); ?></span>
                </div>
            <?php } elseif ( strlen( $msg ) == 5 ) { ?>
                <div class="alert alert-success">
                    <button type="button" aria-hidden="true" class="close">×</button>
                    <span>No se encontraros registros con los criterios: Empresa : <?php echo strtoupper($empresa).' Fechas: ';?> <?php echo substr($fecha_ini,8,2).'/'.substr($fecha_ini,5,2).'/'.substr($fecha_ini,0,4) . ' y ' . substr($fecha_fin,8,2).'/'.substr($fecha_fin,5,2).'/'.substr($fecha_fin,0,4); ?>.</span>
                </div>
            <?php } ?>
